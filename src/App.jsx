...
const PPT_STYLES = [
  { id: "executive", label: "概要把握", desc: "重要ポイントを凝縮・数値ファースト", icon: "⚡", slides: 5 },
  { id: "sales", label: "営業準備", desc: "ヒアリング項目・提案ポイントを前面に", icon: "🤝", slides: 8 },
  { id: "report", label: "社内共有", desc: "根拠データ・ソースURL込みで網羅的に", icon: "📋", slides: 12 },
];

const TOOLTIPS = {
  basicInfo: "企業名はPPT表紙・全スライドのタイトルに使用されます。担当者部署はフッターに表示されます。",
  priorInfo: "商談メモや名刺情報を貼り付けると、AIがリサーチ結果に自動反映します。訪問前のヒアリング内容も有効です。",
  pptStyle: "PPTのスライド構成と詳しさが変わります。概要把握は役員報告向け、営業準備は訪問前ブリーフィング向け、社内共有は詳細報告向けです。",
  industry: "業界を指定すると業界シェア調査の精度が上がります。不明な場合はAIが自動判定するおまかせでOKです。",
  items: `チェックした項目のみ調査します。項目数が多いほど時間がかかります（1項目約1分）。プリセットで用途別に一括選択できます。`,
};
...
const SBR = { navy: "#0D1B2A", cyan: "#00C8FF", lightGray: "#F0F4F8", white: "#FFFFFF", darkNavy: "#1a2744" };
...
function Tooltip({ text }) {
  ...
  return (
    <div style={{ position: "relative", display: "inline-flex", alignItems: "center" }}
      ...
      <div style={{ width: 14, height: 14, borderRadius: "50%", background: "#e5e7eb", display: "flex", alignItems: "center", justifyContent: "center", cursor: "help", fontSize: 9, color: "#6b7280", fontWeight: 700 }}>?</div>
      {show && (
        <div style={{ position: "absolute", left: 20, top: -4, width: 220, background: "#1e293b", color: "#f1f5f9", fontSize: 11, lineHeight: 1.6, padding: "8px 10px", borderRadius: 8, zIndex: 100, boxShadow: "0 4px 16px rgba(0,0,0,0.2)", whiteSpace: "pre-wrap" }}>
          {text}
          <div style={{ position: "absolute", left: -5, top: 8, width: 0, height: 0, borderTop: "5px solid transparent", borderBottom: "5px solid transparent", borderRight: "5px solid #1e293b" }} />
        </div>
      )}
    </div>
  );
...
async function generatePPT(company, dept, results, pptStyle, selectedIds) {
  ...
  const addHeader = (slide, title, sub="") => {
    slide.addShape("rect", {x:0,y:0,w:"100%",h:1.2,fill:{color:"0D1B2A"}});
    slide.addShape("rect", {x:0,y:1.2,w:"100%",h:0.06,fill:{color:"00C8FF"}});
    if(sub) slide.addText(sub,{x:0.3,y:0.08,w:9,h:0.35,fontSize:10,color:"00C8FF"});
    slide.addText(title,{x:0.3,y:sub?0.45:0.3,w:8,h:0.65,fontSize:20,color:"FFFFFF",bold:true});
    slide.addText(company,{x:7.5,y:0.08,w:2.2,h:0.35,fontSize:10,color:"94a3b8",align:"right"});
  };

  const addFooter = (slide, pageNum, total) => {
    slide.addShape("rect",{x:0,y:6.8,w:"100%",h:0.55,fill:{color:"0D1B2A"}});
    slide.addText(`${today}${dept?" | 担当部署："+dept:""}`,{x:0.3,y:6.87,w:7,h:0.35,fontSize:9,color:"64748b"});
    slide.addText(`${pageNum} / ${total}`,{x:8.5,y:6.87,w:1.2,h:0.35,fontSize:9,color:"64748b",align:"right"});
  };

  const truncate = (text, len=80) => text ? (text.length>len ? text.slice(0,len)+"…" : text) : "情報なし";
  const statusLabel = (st) => st==="confirmed"?"✓ 確認済み":st==="partial"?"△ 一部のみ":"⚠ 要確認";
  const statusCol = (st) => st==="confirmed"?"15803d":st==="partial"?"a16207":"dc2626";
  ...
  for(let idx=0;idx<slides.length;idx++) {
    const slideType = slides[idx];
    const slide = prs.addSlide();
    const pageNum = idx+1;
    slide.background = {color:"F0F4F8"};

    if(slideType==="cover") {
      slide.addShape("rect",{x:0,y:0,w:"100%",h:"100%",fill:{color:"0D1B2A"}});
      slide.addShape("rect",{x:0,y:4.0,w:"100%",h:0.06,fill:{color:"00C8FF"}});
      slide.addText("顧客リサーチレポート",{x:0.8,y:1.0,w:8,h:0.6,fontSize:16,color:"00C8FF"});
      slide.addText(company,{x:0.8,y:1.7,w:8,h:1.2,fontSize:36,color:"FFFFFF",bold:true});
      if(dept) slide.addText(`担当部署：${dept}`,{x:0.8,y:3.1,w:6,h:0.4,fontSize:13,color:"94a3b8"});
      slide.addText(`調査日：${today}`,{x:0.8,y:4.3,w:5,h:0.4,fontSize:12,color:"64748b"});
      slide.addText(`スタイル：${style.label}（${slides.length}枚構成）`,{x:0.8,y:4.8,w:6,h:0.35,fontSize:11,color:"475569"});
      slide.addText("SoftBank Robotics Corp.",{x:0.8,y:6.4,w:6,h:0.35,fontSize:10,color:"334155"});

    } else if(slideType==="summary") {
      addHeader(slide,"エグゼクティブサマリー","Executive Summary");
      addFooter(slide,pageNum,total);
      const items=[{id:"company_overview",label:"会社概要"},{id:"pl_summary",label:"売上・利益"},{id:"mid_term_plan",label:"中期経営計画"},{id:"recent_news",label:"直近ニュース"}];
      items.forEach((item,i)=>{
        const r=getR(item.id);
        const x=i%2===0?0.3:5.1, y=1.4+Math.floor(i/2)*2.5;
        slide.addShape("rect",{x,y,w:4.5,h:2.3,fill:{color:"FFFFFF"},line:{color:"e2e8f0",width:0.5}});
        slide.addShape("rect",{x,y,w:4.5,h:0.45,fill:{color:"1a2744"}});
        slide.addText(item.label,{x:x+0.1,y:y+0.07,w:3,h:0.3,fontSize:11,color:"FFFFFF",bold:true});
        if(r?.status) slide.addText(statusLabel(r.status),{x:x+3.0,y:y+0.07,w:1.4,h:0.3,fontSize:9,color:statusCol(r.status),align:"right"});
        slide.addText(truncate(r?.summary,100),{x:x+0.12,y:y+0.55,w:4.26,h:1.65,fontSize:9,color:"374151",valign:"top",wrap:true});
      });

    } else if(slideType==="company") {
      addHeader(slide,"企業基本情報","Company Overview");
      addFooter(slide,pageNum,total);
      [{id:"company_overview",label:"会社概要"},{id:"business_products",label:"事業領域・主力商材"},{id:"group_structure",label:"グループ構造"}].forEach((item,i)=>{
        const r=getR(item.id);
        const y=1.4+i*1.7;
        slide.addShape("rect",{x:0.3,y,w:0.06,h:1.5,fill:{color:"00C8FF"}});
        slide.addText(item.label,{x:0.5,y:y+0.05,w:7,h:0.35,fontSize:12,color:"0D1B2A",bold:true});
        if(r?.status) slide.addText(statusLabel(r.status),{x:7.5,y:y+0.05,w:2,h:0.35,fontSize:10,color:statusCol(r.status),align:"right"});
        slide.addText(truncate(r?.summary,120),{x:0.5,y:y+0.45,w:9.1,h:1.1,fontSize:10,color:"374151",wrap:true});
      });

    } else if(slideType==="finance") {
      addHeader(slide,"財務ハイライト","Financial Highlights");
      addFooter(slide,pageNum,total);
      const r=getR("pl_summary"),rg=getR("growth_profit");
      [{label:"売上高",value:r?.revenue||"—"},{label:"成長率",value:r?.growth||"—"},{label:"営業利益率",value:r?.margin||"—"}].forEach((kpi,i)=>{
        const x=0.3+i*3.1;
        slide.addShape("rect",{x,y:1.4,w:2.8,h:1.3,fill:{color:"0D1B2A"}});
        slide.addShape("rect",{x,y:1.4,w:2.8,h:0.06,fill:{color:"00C8FF"}});
        slide.addText(kpi.label,{x:x+0.1,y:1.5,w:2.6,h:0.35,fontSize:10,color:"94a3b8"});
        slide.addText(String(kpi.value),{x:x+0.1,y:1.85,w:2.6,h:0.7,fontSize:18,color:"00C8FF",bold:true});
      });
      slide.addShape("rect",{x:0.3,y:2.9,w:9.1,h:0.38,fill:{color:"1a2744"}});
      slide.addText("財務サマリー",{x:0.4,y:2.95,w:4,h:0.28,fontSize:11,color:"00C8FF",bold:true});
      slide.addText(truncate(r?.summary,200),{x:0.4,y:3.38,w:9.1,h:1.0,fontSize:10,color:"1f2937",wrap:true});
      if(rg?.summary){
        slide.addShape("rect",{x:0.3,y:4.5,w:9.1,h:0.38,fill:{color:"1a2744"}});
        slide.addText("成長性・収益性",{x:0.4,y:4.55,w:4,h:0.28,fontSize:11,color:"00C8FF",bold:true});
        slide.addText(truncate(rg.summary,200),{x:0.4,y:4.98,w:9.1,h:1.0,fontSize:10,color:"1f2937",wrap:true});
      }

    } else if(slideType==="strategy") {
      addHeader(slide,"戦略・方針","Strategy & Policy");
      addFooter(slide,pageNum,total);
      [{id:"mid_term_plan",label:"中期経営計画"},{id:"dx_strategy",label:"DX・自動化戦略"},{id:"sustainability",label:"サステナビリティ"}].forEach((item,i)=>{
        const r=getR(item.id);
        const y=1.4+i*1.7;
        slide.addShape("rect",{x:0.3,y,w:0.06,h:1.5,fill:{color:"7c3aed"}});
        slide.addText(item.label,{x:0.5,y:y+0.05,w:7,h:0.35,fontSize:12,color:"0D1B2A",bold:true});
        if(r?.status) slide.addText(statusLabel(r.status),{x:7.5,y:y+0.05,w:2,h:0.35,fontSize:10,color:statusCol(r.status),align:"right"});
        slide.addText(truncate(r?.summary,120),{x:0.5,y:y+0.45,w:9.1,h:1.1,fontSize:10,color:"374151",wrap:true});
      });
      ...
      addHeader(slide,"物流特性","Logistics Features");
      addFooter(slide,pageNum,total);
      [{id:"logistics_flow",label:"物流フロー"},{id:"product_features",label:"取扱品の特性"},{id:"existing_systems",label:"既存システム"},{id:"bases_network",label:"拠点ネットワーク"}].forEach((item,i)=>{
        const r=getR(item.id);
        const x=i%2===0?0.3:5.1,y=1.4+Math.floor(i/2)*2.5;
        slide.addShape("rect",{x,y,w:4.5,h:2.3,fill:{color:"FFFFFF"},line:{color:"e2e8f0",width:0.5}});
        slide.addShape("rect",{x,y,w:4.5,h:0.45,fill:{color:"be185d"}});
        slide.addText(item.label,{x:x+0.1,y:y+0.07,w:4.2,h:0.3,fontSize:11,color:"FFFFFF",bold:true});
        slide.addText(truncate(r?.summary,100),{x:x+0.12,y:y+0.55,w:4.26,h:1.65,fontSize:9,color:"374151",valign:"top",wrap:true});
      });

    } else if(slideType==="org") {
      addHeader(slide,"組織・担当者","Organization");
      addFooter(slide,pageNum,total);
      const r=getR("org_structure");
      slide.addShape("rect",{x:0.3,y:1.4,w:9.1,h:0.38,fill:{color:"1a2744"}});
      slide.addText("部署・組織構成",{x:0.4,y:1.45,w:5,h:0.28,fontSize:11,color:"00C8FF",bold:true});
      slide.addText(truncate(r?.summary,300),{x:0.4,y:1.88,w:9.1,h:2.0,fontSize:11,color:"1f2937",wrap:true});

    } else if(slideType==="news") {
      addHeader(slide,"直近のニュース・トピック","Recent News");
      addFooter(slide,pageNum,total);
      const r=getR("recent_news");
      slide.addText(truncate(r?.summary,400),{x:0.4,y:1.5,w:9.1,h:3.0,fontSize:12,color:"1f2937",wrap:true});
      if(r?.source_url) slide.addText(`参照：${r.source_url}`,{x:0.4,y:6.0,w:9.1,h:0.4,fontSize:9,color:"6b7280"});

    } else if(slideType==="market") {
      addHeader(slide,"市場・競合","Market Position");
      addFooter(slide,pageNum,total);
      const r=getR("market_share");
      slide.addShape("rect",{x:0.3,y:1.4,w:9.1,h:0.38,fill:{color:"0f766e"}});
      slide.addText("業界シェア・市場ポジション",{x:0.4,y:1.45,w:6,h:0.28,fontSize:11,color:"FFFFFF",bold:true});
      slide.addText(truncate(r?.summary,300),{x:0.4,y:1.88,w:9.1,h:2.5,fontSize:11,color:"1f2937",wrap:true});

    } else if(slideType==="proposal") {
      addHeader(slide,"提案ポイント","Proposal Points");
      addFooter(slide,pageNum,total);
      ...
      slide.addShape("rect",{x:0.3,y:1.4,w:4.4,h:0.38,fill:{color:"15803d"}});
      slide.addText("✓ 確認済み情報",{x:0.4,y:1.45,w:4,h:0.28,fontSize:11,color:"FFFFFF",bold:true});
      confirmedItems.slice(0,5).forEach((item,i)=>{
        slide.addText(`• ${item.label}`,{x:0.5,y:1.95+i*0.55,w:4.1,h:0.45,fontSize:10,color:"1f2937"});
      });
      slide.addShape("rect",{x:5.1,y:1.4,w:4.4,h:0.38,fill:{color:"dc2626"}});
      slide.addText("⚠ ヒアリング必須",{x:5.2,y:1.45,w:4,h:0.28,fontSize:11,color:"FFFFFF",bold:true});
      unconfirmedItems.slice(0,5).forEach((item,i)=>{
        slide.addText(`• ${item.label}`,{x:5.2,y:1.95+i*0.55,w:4.1,h:0.45,fontSize:10,color:"1f2937"});
      });

    } else if(slideType==="hearing") {
      addHeader(slide,"ヒアリングシート","Hearing Sheet");
      addFooter(slide,pageNum,total);
      const hearingItems=ALL_ITEMS.filter(i=>selectedIds.has(i.id)&&(results[i.id]?.status==="unconfirmed"||results[i.id]?.status==="partial"));
      slide.addText(`要確認事項（${hearingItems.length}件）`,{x:0.3,y:1.35,w:9,h:0.4,fontSize:13,color:"0D1B2A",bold:true});
      let yPos=1.85;
      hearingItems.slice(0,8).forEach((item)=>{
        const r=results[item.id];
        const isU=r?.status==="unconfirmed";
        slide.addShape("rect",{x:0.3,y:yPos,w:0.06,h:0.55,fill:{color:isU?"dc2626":"a16207"}});
        slide.addText(item.label,{x:0.5,y:yPos+0.02,w:2.5,h:0.25,fontSize:10,color:"0D1B2A",bold:true});
        slide.addText(truncate(r?.missing||"訪問時に確認",80),{x:0.5,y:yPos+0.28,w:9.1,h:0.25,fontSize:9,color:"4b5563"});
        yPos+=0.65;
      });

    } else if(slideType==="sources") {
      addHeader(slide,"情報ソース一覧","References");
      addFooter(slide,pageNum,total);
      const sourceItems=ALL_ITEMS.filter(i=>selectedIds.has(i.id)&&results[i.id]?.source_url);
      slide.addText("本レポートの参照元URL",{x:0.3,y:1.35,w:9,h:0.4,fontSize:13,color:"0D1B2A",bold:true});
      sourceItems.slice(0,10).forEach((item,i)=>{
        const r=results[item.id];
        slide.addText(`${item.label}：${truncate(r.source_url,80)}`,{x:0.4,y:1.88+i*0.48,w:9.1,h:0.4,fontSize:9,color:"374151",wrap:true});
      });
    }
  }
...
function SlidePreview({ company, dept, results, pptStyle, selectedIds }) {
  ...
  const headerStyle = { background:SBR.navy, padding:"10px 16px", borderBottom:`3px solid ${SBR.cyan}`, flexShrink:0 };

  const renderSlide = () => {
    if(slideType==="cover") return (
      <div style={{ background:SBR.navy, height:"100%", display:"flex", flexDirection:"column", justifyContent:"center", padding:"28px" }}>
        <div style={{ fontSize:11, color:SBR.cyan, marginBottom:6 }}>顧客リサーチレポート</div>
        <div style={{ fontSize:28, fontWeight:700, color:SBR.white, marginBottom:6, lineHeight:1.2 }}>{company}</div>
        <div style={{ width:50, height:3, background:SBR.cyan, marginBottom:14 }}/>
        {dept&&<div style={{ fontSize:12, color:"#94a3b8", marginBottom:4 }}>担当部署：{dept}</div>}
        <div style={{ fontSize:11, color:"#64748b" }}>調査日：{new Date().toLocaleDateString("ja-JP")}</div>
        <div style={{ fontSize:10, color:"#475569", marginTop:6 }}>{PPT_STYLES.find(s=>s.id===pptStyle)?.label}（{slides.length}枚構成）</div>
      </div>
    );
    ...
    if(slideType==="summary") return (
      <div style={{ height:"100%", display:"flex", flexDirection:"column" }}>
        <div style={headerStyle}>
          <div style={{ fontSize:9, color:SBR.cyan }}>Executive Summary</div>
          <div style={{ fontSize:14, fontWeight:700, color:SBR.white }}>エグゼクティブサマリー</div>
        </div>
        <div style={{ flex:1, padding:"10px", display:"grid", gridTemplateColumns:"1fr 1fr", gap:6, overflow:"hidden" }}>
          {[{id:"company_overview",label:"会社概要"},{id:"pl_summary",label:"売上・利益"},{id:"mid_term_plan",label:"中期経営計画"},{id:"recent_news",label:"直近ニュース"}].map(item=>{
            const r=getR(item.id);
            return(
              <div key={item.id} style={{ background:"#fff", borderRadius:4, overflow:"hidden", border:"1px solid #e2e8f0" }}>
                <div style={{ background:SBR.darkNavy, padding:"3px 8px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <span style={{ fontSize:9, color:SBR.white, fontWeight:600 }}>{item.label}</span>
                  {r?.status&&<span style={{ fontSize:8, color:statusColor(r.status) }}>{statusLabel(r.status)}</span>}
                </div>
                <div style={{ padding:"5px 8px", fontSize:9, color:"#374151", lineHeight:1.5, overflow:"hidden" }}>{truncate(r?.summary,90)}</div>
              </div>
            );
          })}
        </div>
    ...
    if(slideType==="finance") return (
      <div style={{ height:"100%", display:"flex", flexDirection:"column" }}>
        <div style={headerStyle}>
          <div style={{ fontSize:9, color:SBR.cyan }}>Financial Highlights</div>
          <div style={{ fontSize:14, fontWeight:700, color:SBR.white }}>財務ハイライト</div>
        </div>
        <div style={{ flex:1, padding:"10px", overflow:"hidden" }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:6, marginBottom:8 }}>
            {[{label:"売上高",value:getR("pl_summary")?.revenue||"—"},{label:"成長率",value:getR("pl_summary")?.growth||"—"},{label:"営業利益率",value:getR("pl_summary")?.margin||"—"}].map((kpi,i)=>(
              <div key={i} style={{ background:SBR.navy, borderRadius:4, padding:"8px", borderTop:`3px solid ${SBR.cyan}` }}>
                <div style={{ fontSize:9, color:"#94a3b8", marginBottom:3 }}>{kpi.label}</div>
                <div style={{ fontSize:16, fontWeight:700, color:SBR.cyan }}>{kpi.value}</div>
              </div>
            ))}
          </div>
          <div style={{ background:"#fff", borderRadius:4, padding:8, border:"1px solid #e2e8f0" }}>
            <div style={{ fontSize:9, fontWeight:600, color:SBR.darkNavy, marginBottom:3 }}>財務サマリー</div>
            <div style={{ fontSize:9, color:"#374151", lineHeight:1.5, overflow:"hidden" }}>{truncate(getR("pl_summary")?.summary,150)}</div>
          </div>
        </div>
      </div>
    );
    ...
    if(slideType==="strategy") return (
      <div style={{ height:"100%", display:"flex", flexDirection:"column" }}>
        <div style={headerStyle}>
          <div style={{ fontSize:9, color:SBR.cyan }}>Strategy & Policy</div>
          <div style={{ fontSize:14, fontWeight:700, color:SBR.white }}>戦略・方針</div>
        </div>
        <div style={{ flex:1, padding:"10px", overflow:"hidden" }}>
          {[{id:"mid_term_plan",label:"中期経営計画"},{id:"dx_strategy",label:"DX・自動化戦略"},{id:"sustainability",label:"サステナビリティ"}].map((item,i)=>{
            const r=getR(item.id);
            return(
              <div key={i} style={{ display:"flex", gap:6, marginBottom:7, background:"#fff", borderRadius:4, overflow:"hidden", border:"1px solid #e2e8f0" }}>
                <div style={{ width:3, background:SBR.cyan, flexShrink:0 }}/>
                <div style={{ padding:"6px 6px 6px 0", overflow:"hidden" }}>
                  <div style={{ fontSize:10, fontWeight:600, color:SBR.darkNavy, marginBottom:2 }}>{item.label}</div>
                  <div style={{ fontSize:9, color:"#374151", lineHeight:1.4, overflow:"hidden" }}>{truncate(r?.summary,100)}</div>
                </div>
              </div>
            );
          })}
    ...
    if(slideType==="logistics") return (
      <div style={{ height:"100%", display:"flex", flexDirection:"column" }}>
        <div style={headerStyle}>
          <div style={{ fontSize:9, color:SBR.cyan }}>Logistics Features</div>
          <div style={{ fontSize:14, fontWeight:700, color:SBR.white }}>物流特性</div>
        </div>
        <div style={{ flex:1, padding:"10px", display:"grid", gridTemplateColumns:"1fr 1fr", gap:6, overflow:"hidden" }}>
          {[{id:"logistics_flow",label:"物流フロー"},{id:"product_features",label:"取扱品の特性"},{id:"existing_systems",label:"既存システム"},{id:"bases_network",label:"拠点ネットワーク"}].map((item,i)=>{
            const r=getR(item.id);
            return(
              <div key={i} style={{ background:"#fff", borderRadius:4, overflow:"hidden", border:"1px solid #e2e8f0" }}>
                <div style={{ background:"#be185d", padding:"3px 8px" }}>
                  <span style={{ fontSize:9, color:"#fff", fontWeight:600 }}>{item.label}</span>
                </div>
                <div style={{ padding:"5px 8px", fontSize:9, color:"#374151", lineHeight:1.5, overflow:"hidden" }}>{truncate(r?.summary,80)}</div>
              </div>
            );
          })}
        </div>
    ...
    if(slideType==="company") return (
      <div style={{ height:"100%", display:"flex", flexDirection:"column" }}>
        <div style={headerStyle}>
          <div style={{ fontSize:9, color:SBR.cyan }}>Company Overview</div>
          <div style={{ fontSize:14, fontWeight:700, color:SBR.white }}>企業基本情報</div>
        </div>
        <div style={{ flex:1, padding:"10px", overflow:"hidden" }}>
          {[{id:"company_overview",label:"会社概要"},{id:"business_products",label:"事業領域"},{id:"group_structure",label:"グループ構造"}].map((item,i)=>{
            const r=getR(item.id);
            return(
              <div key={i} style={{ display:"flex", gap:6, marginBottom:7, background:"#fff", borderRadius:4, overflow:"hidden", border:"1px solid #e2e8f0" }}>
                <div style={{ width:3, background:SBR.cyan, flexShrink:0 }}/>
                <div style={{ padding:"6px 6px 6px 0" }}>
                  <div style={{ fontSize:10, fontWeight:600, color:SBR.darkNavy, marginBottom:2 }}>{item.label}</div>
                  <div style={{ fontSize:9, color:"#374151", lineHeight:1.4 }}>{truncate(r?.summary,100)}</div>
                </div>
              </div>
            );
          })}
    ...
    if(slideType==="org") return (
      <div style={{ height:"100%", display:"flex", flexDirection:"column" }}>
        <div style={headerStyle}>
          <div style={{ fontSize:9, color:SBR.cyan }}>Organization</div>
          <div style={{ fontSize:14, fontWeight:700, color:SBR.white }}>組織・担当者</div>
        </div>
        <div style={{ flex:1, padding:"10px" }}>
          <div style={{ background:"#fff", borderRadius:4, padding:10, border:"1px solid #e2e8f0" }}>
            <div style={{ fontSize:10, fontWeight:600, color:SBR.darkNavy, marginBottom:4 }}>部署・組織構成</div>
            <div style={{ fontSize:10, color:"#374151", lineHeight:1.6 }}>{truncate(getR("org_structure")?.summary,200)}</div>
          </div>
        </div>
      </div>
    );

    if(slideType==="news") return (
      <div style={{ height:"100%", display:"flex", flexDirection:"column" }}>
        <div style={headerStyle}>
          <div style={{ fontSize:9, color:SBR.cyan }}>Recent News</div>
          <div style={{ fontSize:14, fontWeight:700, color:SBR.white }}>直近のニュース・トピック</div>
        </div>
        <div style={{ flex:1, padding:"10px" }}>
          <div style={{ fontSize:10, color:"#374151", lineHeight:1.7 }}>{truncate(getR("recent_news")?.summary,300)}</div>
        </div>
      </div>
    );

    if(slideType==="market") return (
      <div style={{ height:"100%", display:"flex", flexDirection:"column" }}>
        <div style={headerStyle}>
          <div style={{ fontSize:9, color:SBR.cyan }}>Market Position</div>
          <div style={{ fontSize:14, fontWeight:700, color:SBR.white }}>市場・競合</div>
        </div>
        <div style={{ flex:1, padding:"10px" }}>
          <div style={{ background:"#fff", borderRadius:4, padding:10, border:"1px solid #e2e8f0" }}>
            <div style={{ fontSize:10, fontWeight:600, color:"#0f766e", marginBottom:4 }}>業界シェア・市場ポジション</div>
            <div style={{ fontSize:10, color:"#374151", lineHeight:1.6 }}>{truncate(getR("market_share")?.summary,200)}</div>
          </div>
        </div>
      </div>
    );

    if(slideType==="proposal") return (
      <div style={{ height:"100%", display:"flex", flexDirection:"column" }}>
        <div style={headerStyle}>
          <div style={{ fontSize:9, color:SBR.cyan }}>Proposal Points</div>
          <div style={{ fontSize:14, fontWeight:700, color:SBR.white }}>提案ポイント</div>
        </div>
        <div style={{ flex:1, padding:"10px", display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, overflow:"hidden" }}>
          <div>
            <div style={{ background:"#15803d", borderRadius:4, padding:"3px 8px", marginBottom:6 }}>
              <span style={{ fontSize:9, color:"#fff", fontWeight:600 }}>✓ 確認済み情報</span>
            </div>
            {ALL_ITEMS.filter(i=>selectedIds.has(i.id)&&results[i.id]?.status==="confirmed").slice(0,5).map((item,i)=>(
              <div key={i} style={{ fontSize:9, color:"#374151", padding:"2px 0", borderBottom:"1px solid #f3f4f6" }}>• {item.label}</div>
            ))}
          </div>
          <div>
            <div style={{ background:"#dc2626", borderRadius:4, padding:"3px 8px", marginBottom:6 }}>
              <span style={{ fontSize:9, color:"#fff", fontWeight:600 }}>⚠ ヒアリング必須</span>
            </div>
            {ALL_ITEMS.filter(i=>selectedIds.has(i.id)&&results[i.id]?.status==="unconfirmed").slice(0,5).map((item,i)=>(
              <div key={i} style={{ fontSize:9, color:"#374151", padding:"2px 0", borderBottom:"1px solid #f3f4f6" }}>• {item.label}</div>
            ))}
          </div>
        </div>
    ...
    if(slideType==="hearing") return (
      <div style={{ height:"100%", display:"flex", flexDirection:"column" }}>
        <div style={headerStyle}>
          <div style={{ fontSize:9, color:SBR.cyan }}>Hearing Sheet</div>
          <div style={{ fontSize:14, fontWeight:700, color:SBR.white }}>ヒアリングシート</div>
        </div>
        <div style={{ flex:1, padding:"10px", overflow:"hidden" }}>
          {ALL_ITEMS.filter(i=>selectedIds.has(i.id)&&(results[i.id]?.status==="unconfirmed"||results[i.id]?.status==="partial")).slice(0,7).map((item,i)=>{
            ...
            return(
              <div key={i} style={{ display:"flex", gap:6, marginBottom:5, alignItems:"flex-start" }}>
                <div style={{ width:3, height:30, background:r?.status==="unconfirmed"?"#dc2626":"#a16207", borderRadius:2, flexShrink:0, marginTop:2 }}/>
                <div>
                  <div style={{ fontSize:9, fontWeight:600, color:SBR.darkNavy }}>{item.label}</div>
                  <div style={{ fontSize:8, color:"#6b7280" }}>{truncate(r?.missing||"訪問時に確認",60)}</div>
                </div>
              </div>
            );
          })}
    ...
    if(slideType==="sources") return (
      <div style={{ height:"100%", display:"flex", flexDirection:"column" }}>
        <div style={headerStyle}>
          <div style={{ fontSize:9, color:SBR.cyan }}>References</div>
          <div style={{ fontSize:14, fontWeight:700, color:SBR.white }}>情報ソース一覧</div>
        </div>
        <div style={{ flex:1, padding:"10px", overflow:"hidden" }}>
          {ALL_ITEMS.filter(i=>selectedIds.has(i.id)&&results[i.id]?.source_url).slice(0,8).map((item,i)=>(
            <div key={i} style={{ fontSize:9, color:"#374151", padding:"3px 0", borderBottom:"1px solid #f3f4f6" }}>
              <span style={{ fontWeight:600 }}>{item.label}：</span>{truncate(results[item.id].source_url,70)}
            </div>
          ))}
        </div>
      </div>
    ...
    return <div style={{ padding:16, fontSize:12, color:"#374151" }}>スライドを準備中...</div>;
  };

  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%" }}>
      <div style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", padding:"12px", background:"#e2e8f0" }}>
        <div style={{ width:"100%", maxWidth:580, aspectRatio:"16/9", background:SBR.lightGray, borderRadius:8, boxShadow:"0 4px 20px rgba(0,0,0,0.15)", overflow:"hidden", display:"flex", flexDirection:"column" }}>
          {renderSlide()}
        </div>
      </div>

      {/* ナビゲーション */}
      <div style={{ padding:"10px 16px", background:"#fff", borderTop:"1px solid #e5e7eb", display:"flex", alignItems:"center", justifyContent:"space-between", gap:8 }}>
        <button onClick={()=>setCurrentSlide(s=>Math.max(0,s-1))} disabled={currentSlide===0}
          style={{ padding:"6px 14px", borderRadius:6, border:"1px solid #e5e7eb", background:currentSlide===0?"#f9fafb":"#fff", cursor:currentSlide===0?"not-allowed":"pointer", fontSize:12, color:currentSlide===0?"#9ca3af":"#374151", fontFamily:"inherit" }}>
          ← 前へ
        </button>
        ...
        <div style={{ display:"flex", gap:5, alignItems:"center" }}>
          {slides.map((_,i)=>(
            <div key={i} onClick={()=>setCurrentSlide(i)}
              style={{ width:i===currentSlide?18:7, height:7, borderRadius:99, background:i===currentSlide?SBR.navy:"#d1d5db", cursor:"pointer", transition:"all 0.2s" }}/>
          ))}
        </div>

        <div style={{ display:"flex", gap:8, alignItems:"center" }}>
          <span style={{ fontSize:11, color:"#6b7280" }}>{currentSlide+1} / {total}</span>
          <button onClick={()=>setCurrentSlide(s=>Math.min(total-1,s+1))} disabled={currentSlide===total-1}
            style={{ padding:"6px 14px", borderRadius:6, border:"1px solid #e5e7eb", background:currentSlide===total-1?"#f9fafb":"#fff", cursor:currentSlide===total-1?"not-allowed":"pointer", fontSize:12, color:currentSlide===total-1?"#9ca3af":"#374151", fontFamily:"inherit" }}>
            次へ →
          </button>
          <button onClick={async()=>{setGenerating(true);try{await generatePPT(company,dept,results,pptStyle,selectedIds);}catch(e){alert("PPT生成エラー: "+e.message);}setGenerating(false);}}
            disabled={generating}
            style={{ padding:"7px 18px", borderRadius:6, border:"none", background:generating?"#d1d5db":SBR.navy, color:"#fff", cursor:generating?"not-allowed":"pointer", fontSize:12, fontWeight:700, fontFamily:"inherit" }}>
  ...
  return (
    <div style={{background:"#f8fafc",border:"1px solid #e2e8f0",borderRadius:10,padding:"10px 14px",marginBottom:8}}>
      ...
      <svg viewBox="0 0 300 82" width="100%" style={{display:"block",background:"#f1f5f9",borderRadius:8}}>
        ...
        <clipPath id="wbc6"><rect x="8" y="50" width="120" height="14"/></clipPath>
        <g clipPath="url(#wbc6)">{Array.from({length:9}).map((_,i)=><line key={i} x1={8+((i*16-beltOffset+120)%120)} y1="50" x2={8+((i*16-beltOffset+120)%120)} y2="64" stroke="#475569" strokeWidth="1.5"/>)}</g>
...
// セクションラベル（ツールチップ付き）
function SectionLabel({ label, tooltip }) {
  return (
      ...
      <div style={{display:"flex",height:"calc(100vh - 50px)"}}>
        <div style={{width:292,background:"#fff",borderRight:"1px solid #e5e7eb",overflowY:"auto",display:"flex",flexDirection:"column",flexShrink:0}}>
          <div style={{padding:"14px 14px 0",flex:1}}>
            ...
            <SectionLabel label="事前情報" tooltip={TOOLTIPS.priorInfo}/>
            <span style={{fontWeight:400,color:"#9ca3af",fontSize:10,marginTop:-6,display:"block",marginBottom:4}}>任意</span>
            ...
            <div style={{height:1,background:"#f3f4f6",margin:"10px 0"}}/>
            <SectionLabel label="業界シェアの調査軸" tooltip={TOOLTIPS.industry}/>
            ...
            <div style={{height:1,background:"#f3f4f6",margin:"10px 0"}}/>
            <SectionLabel label={`調査項目　${selected.size}件選択`} tooltip={TOOLTIPS.items}/>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
          {!hasResults?(
            ...
            <>
              <div style={{background:"#fff",borderBottom:"1px solid #e5e7eb",padding:"0 20px",display:"flex",alignItems:"center"}}>
